...
function stateSubText(state: EventState, event: EventBooking, d: EventsPageDict): string {
  ...
  switch (state) {
    ...
    case "draft": return `Pasul ${event.orders?.length ?? 4} din 6`;
    case "past": return total > 0 ? "Plăți finalizate" : "Eveniment finalizat";
    case "cancelled": {
      const refund = pickAmount(event.refundAmount);
      return refund > 0 ? `Rambursat: ${new Intl.NumberFormat("ro-MD").format(refund)} MDL` : "Anulat";
    }
    default: return "";
  }
}
          ...
          <div className="mx-auto max-w-[1180px] px-4 sm:px-6 lg:px-8 2xl:px-0 py-14">
            ...
            <div className="mb-8">
              <div className="flex items-start justify-between gap-4 mb-3">
                <h1 className="text-[40px] font-semibold text-[#f0f0f0] font-figtree tracking-tight leading-none">
                  {d.title}
                </h1>
                <Link
                  href={`/${locale}/booking`}
            ...
            <div className="flex gap-2 mb-6">
              {tabs.map(({ key, label }) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium font-figtree tracking-tight border transition-colors cursor-pointer ${
                    filter === key
...
function EventRow({
  ...
  if (primary?.orderNumber) parts.push(primary.orderNumber);

  return (
    <div className="flex items-center gap-6 px-5 py-4 border-b border-[#141414] last:border-b-0 bg-[#0c0c0c] hover:bg-[#0f0f0f] transition-colors group">
      {/* Date */}
      <div className="w-16 shrink-0 flex flex-col">
        <span className="text-[22px] font-semibold text-[#f0f0f0] font-figtree tracking-tight leading-none">
          {day}
        </span>
        <span className="text-xs text-[#555] font-figtree tracking-tight mt-0.5">{year}</span>
      </div>
      ...
      <Link
        ...
        className="shrink-0 px-4 py-2 border border-[#2a2a2a] text-sm font-medium text-[#c0c0c0] font-figtree tracking-tight hover:border-[#4a4a4a] hover:text-[#f0f0f0] transition-colors"
      >
        {viewLabel}
      </Link>
    </div>
